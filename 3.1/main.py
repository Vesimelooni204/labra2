...
class PaskaPituus(Exception):
    def __init__(self, message, errors):
        super().__init__(message)
        self.errors = errors
def avgVarErr(arvot, acc):
    ...
    l = len(arvot[0])
    p = len(arvot) # Mittausten määrä
    # Check
    for i in arvot:
        if len(i) != l:
            raise PaskaPituus(f"Listan pituus on epähyvä: {i}")
        else:
            pass
...
sovfuc_min = (sov[0] - a_err) * dis
sovfuc_max = (sov[0] + a_err) * dis
...
# Mittauspisteiden keskiarvo ja virhepalkit
